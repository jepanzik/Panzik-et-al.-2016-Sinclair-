...
Stereonet plot of the measured inclination & declination values. The a95 circles are not perfect, but do a good job estimating the uncertainty.
...
p <- ggplot(cong) +
  ...
  scale_radius(range=c(f*min(cong$a95),f*max(cong$a95))) +
  theme_bw() +
...
We can now put the $R$ and $R_0$ values for each in a table to determine whether sites pass the randomness test.
